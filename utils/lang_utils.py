...
def show_similar(sv, sv_vector):
    ...
    similar_accounts = sv.get_similar(sv_vector, topn=15)
    similar_accounts.drop_duplicates(subset='screen_name', inplace=True)
    similar_accounts = similar_accounts[~similar_accounts['name'].isin(selected_accounts)]

    st.write(similar_accounts[['name','screen_name','description','similarity','twitter_id']].head(5))
    return
